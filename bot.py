...
async def immune(interaction: discord.Interaction, member: discord.Member):
    ...
    save_immune_users()
    await interaction.response.send_message(f"{member.mention} is now IMMUNE 🛡️")
...
async def unimmune(interaction: discord.Interaction, member: discord.Member):
    if str(member.id) in immune_users:
        ...
        save_immune_users()
        await interaction.response.send_message(f"{member.mention} is no longer immune ❌")
    else:
        await interaction.response.send_message("User is not immune.", ephemeral=True)

@bot.tree.command(name="kick", description="Kick a member")
@commands.has_permissions(kick_members=True)
async def kick(interaction: discord.Interaction, member: discord.Member, reason: str = None):
    if str(member.id) in immune_users:
        return await interaction.response.send_message("This user is IMMUNE 🛡️", ephemeral=True)
...
async def ban(interaction: discord.Interaction, member: discord.Member, reason: str = None):
    ...
    if member == interaction.guild.owner:
        return await interaction.response.send_message("You can't ban the owner.", ephemeral=True)
    try:
...
async def timeout(interaction: discord.Interaction, member: discord.Member, minutes: int):
    ...
    if member == interaction.guild.owner:
        return await interaction.response.send_message("You can't timeout the owner.", ephemeral=True)
    try:
...
if __name__ == "__main__":
    try:
        ...
        if e.status == 429:
            print("Rate limited by Discord. Try again in a few minutes.")
            logging.error("Discord rate limited the bot.")
